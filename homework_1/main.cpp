...
void printClassList(Student *classList, int classSize)
{
    ...
    for (int person = 0; person < classSize; person++)
    {
       ...
       cout<<person+1<<". "<<setw(3)<< classList[person].fname << " "
           ...
           // cout<<"# of spaces: "<<max-classList[person].lname.length()-classList[person].fname.length();
        
        for(int i=0; i<max-classList[person].lname.length()-classList[person].fname.length();i++)    
           { cout<<" ";}
        cout<<":";
         ...
         for(int j=0;j<classList[person].n;j++){
         cout <<setw(6)<< classList[person].grades[j] << " ";
         }
         cout<<endl;
            
             //<< classList[person].grades[1] << endl;
    }
}
...
int main()
{
    ...
    // of `Student` structs.
    //cout<<"before call";
   classSize = loadClassList(classList, "student.txt");
   
    // Function returned how many lines (students) read in, lets print it.
    cout << "Leila Kalantari"<< endl;
